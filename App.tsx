...
export default function App() {
  ...
  const [announcement, setAnnouncement] = useState("Welcome to our new collection! Stay tuned for daily updates.");
  ...
  const [requestReleaseDate, setRequestReleaseDate] = useState('');
  const [requestViewerName, setRequestViewerName] = useState('');
            ...
            if (event.lengthComputable) {
              ...
              if (percent !== lastPercent) {
                ...
                if (elapsedMs > 500) {
                  ...
                  const uploadedStr = (event.loaded / (1024 * 1024)).toFixed(1);
                  const totalStr = (event.total / (1024 * 1024)).toFixed(1);

                  setUploadStats({
                    speed: `${speedMbps} MB/s`,
                    eta: etaStr,
                    uploadedSize: `${uploadedStr} MB / ${totalStr} MB`
                  });
                }
              }
            }
  ...
  const submitRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    const empty: string[] = [];
    if (!requestTitle.trim()) empty.push('title');
    if (!requestReleaseDate.trim()) empty.push('releaseDate');
    if (!requestViewerName.trim()) empty.push('viewerName');

    if (empty.length > 0) {
      setEmptyFields(empty);
      setTimeout(() => setEmptyFields([]), 1000);
      return;
    }
    try {
      const res = await fetch('/api/requests', {
        ...
        body: JSON.stringify({ title: requestTitle, releaseDate: requestReleaseDate, viewerName: requestViewerName })
      });
      ...
      if (res.ok) {
        ...
        setRequestTitle('');
        setRequestReleaseDate('');
        setRequestViewerName('');
      ...
      <main className="flex-1 p-6 md:p-10 flex flex-col gap-8 overflow-y-auto">
        ...
        {isAdmin ? (
          <div className="md:hidden flex flex-wrap gap-2">
            <button 
              onClick={() => setShowUpload(true)}
              className="flex-1 bg-[#d4af37] text-black font-bold py-2.5 rounded text-xs uppercase flex items-center justify-center gap-2 min-w-[100px]"
            >
              <Plus className="w-4 h-4" /> Upload
            </button>
            <button 
              onClick={() => setShowAdminRequests(true)}
              className="flex-1 bg-[#1a1a1a] text-white border border-[#333] font-bold py-2.5 rounded text-xs uppercase hover:bg-[#222] transition-colors flex items-center justify-center gap-2 relative min-w-[100px]"
            >
          ...
          <div className="md:hidden flex gap-2">
            <button 
              onClick={() => setShowRequestForm(true)}
              className="flex-1 bg-[#1a1a1a] text-white border border-[#333] hover:bg-[#222] font-bold py-2.5 rounded text-xs uppercase transition-colors flex items-center justify-center gap-2"
            >
              <MessageSquarePlus className="w-4 h-4" /> Request
            </button>
          </div>
        )}
        ...
        <header id="latest-releases" className="flex flex-col sm:flex-row sm:justify-between sm:items-end border-b border-[#1a1a1a] pb-6 gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-light tracking-tight m-0 text-white">Condition</h1>
            <p className="text-[#666] mt-2 text-sm max-w-2xl">If you need a specific movie, go to the 'Request' option above, enter the movie title, release date, and your name, then submit the request; the movie will be uploaded here within 24 hours. If it cannot be found, you will be notified here, so please check back after 24 hours. Simply click the 'Download' option to start the download; the link remains valid for 2 days, so please download it within that timeframe.</p>
          </div>
        ...
        {/* Admin Announcement */}
        <div className="bg-[#0d0d0d] border border-[#d4af37]/20 rounded-xl p-6 text-[#d4af37] text-sm flex items-center gap-4">
          <div className="text-xl">📢</div>
          <div className="flex-1">
            <h4 className="font-bold text-white mb-1 uppercase tracking-wider text-[10px]">Admin Announcement</h4>
            {isAdmin ? (
               <input 
                  type="text" 
                  value={announcement} 
                  onChange={(e) => setAnnouncement(e.target.value)} 
                  className="w-full bg-[#1a1a1a] border border-[#333] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#d4af37]"
               />
            ) : (
               <p>{announcement}</p>
            )}
        ...
        <footer className="sticky bottom-0 z-20 bg-[#050505]/95 backdrop-blur-md border-t border-[#1a1a1a] py-4 px-6 md:px-10 -mx-6 md:-mx-10 mt-auto flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <form onSubmit={handleUpload} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedVideoFile && (
                      <div className="text-xs text-[#d4af37] bg-[#d4af37]/5 border border-[#d4af37]/20 rounded p-2.5 flex flex-col gap-1.5">
                        ...
                        {isUploading && (
                          <div className="space-y-1.5 mt-2 pt-2 border-t border-[#d4af37]/10">
                            ...
                            {uploadStats && (
                              <div className="grid grid-cols-3 gap-1.5 mt-2 pt-1 text-[10px] text-[#888]">
                                <div className="bg-white/5 rounded p-1.5 text-center">
                                  <div className="text-[8px] uppercase tracking-wider text-[#555]">Speed</div>
                                  <div className="font-semibold text-white mt-0.5">{uploadStats.speed}</div>
                                </div>
                                <div className="bg-white/5 rounded p-1.5 text-center">
                                  <div className="text-[8px] uppercase tracking-wider text-[#555]">Remaining</div>
                                  <div className="font-semibold text-white mt-0.5">{uploadStats.eta}</div>
                                </div>
                                <div className="bg-white/5 rounded p-1.5 text-center">
                                  <div className="text-[8px] uppercase tracking-wider text-[#555]">Uploaded</div>
                                  <div className="font-semibold text-white mt-0.5 truncate">{uploadStats.uploadedSize.split(" / ")[0]}</div>
                                </div>
                              </div>
                            )}
                          </div>
              ...
              <div className="pt-4 flex flex-row-reverse gap-3">
                <button 
                  ...
                  className="flex-[2] bg-[#d4af37] text-black font-bold py-3 rounded text-sm uppercase transition-colors hover:bg-[#b5952f] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isUploading ? (
                    <>
                ...
                <button 
                  type="button"
                  onClick={handleCancelUpload}
                  className="flex-1 bg-transparent hover:bg-white/5 text-[#888] hover:text-white border border-[#1a1a1a] font-bold py-3 rounded text-sm uppercase transition-colors flex items-center justify-center"
                >
                  Cancel
                </button>
              </div>
            ...
            <form onSubmit={submitRequest}>
              <div className="mb-4">
                ...
                <input 
                  ...
                  className={`w-full bg-[#050505] border ${emptyFields.includes('title') ? 'border-red-500 animate-blink-red' : 'border-[#1a1a1a]'} rounded px-4 py-3 text-white focus:outline-none focus:border-[#d4af37] transition-colors text-sm`}
                  placeholder="What movie do you want?"
                />
              </div>
              <div className="mb-4">
                <label className="block text-xs uppercase tracking-wider text-[#888] mb-2">Release Date</label>
                <input 
                  type="text" 
                  value={requestReleaseDate}
                  onChange={(e) => setRequestReleaseDate(e.target.value)}
                  className={`w-full bg-[#050505] border ${emptyFields.includes('releaseDate') ? 'border-red-500 animate-blink-red' : 'border-[#1a1a1a]'} rounded px-4 py-3 text-white focus:outline-none focus:border-[#d4af37] transition-colors text-sm`}
                  placeholder="e.g. 2025"
                />
              </div>
              <div className="mb-6">
                <label className="block text-xs uppercase tracking-wider text-[#888] mb-2">Your Name</label>
                <input 
                  type="text" 
                  value={requestViewerName}
                  onChange={(e) => setRequestViewerName(e.target.value)}
                  className={`w-full bg-[#050505] border ${emptyFields.includes('viewerName') ? 'border-red-500 animate-blink-red' : 'border-[#1a1a1a]'} rounded px-4 py-3 text-white focus:outline-none focus:border-[#d4af37] transition-colors text-sm`}
                  ...
                  <div key={req.id} className="bg-[#111] border border-[#222] rounded p-4 flex justify-between items-start gap-4 group hover:border-[#333] transition-colors">
                    <div className="flex-1">
                      <h4 className="text-white font-medium mb-1 flex items-center gap-2">
                        {req.title} {req.releaseDate && <span className="text-[#d4af37]">({req.releaseDate})</span>}
                      </h4>
                      <div className="text-[10px] text-[#666] uppercase tracking-wider flex items-center gap-2 flex-wrap mb-2">
                        <span>By: {req.viewerName || 'Anonymous'}</span>
                        <span>•</span>
